...
import type MercariCategory from "@/types/MercariCategory";
    ...
    categories() {
      if (this.list.length) {
        ...
        return !this.category_id
          ? list.filter((item: MercariCategory) => item.level == "0")
          : list.filter((item: MercariCategory) => item.parent_id == this.category_id);
      } else return [];
    },
  },
  created() {
    useCategoryOtherMarketStore().fetch("mercari");
  },
  ...
  methods: {
    ...
    handleChooseCategory(id: string) {
      ...
      if (!this.categories.length) {
        this.$router.push({
          path: "/other-markets/shop/mercari",
          query: { category_id: id },
        });
      } else {
        this.$router.push({
          path: "/other-markets/mercari",
          query: {
            category_id: id,
          },
        });
